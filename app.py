...
from data import load_wti, add_gdelt_features
...
with st.sidebar:
    ...
    include_gdelt = st.checkbox("Include GDELT geopolitical/news signals", value=False)
    backtest_start_year = st.slider("Backtest start year", 2012, 2023, 2015)
    refresh = st.button("Refresh data / retrain")

@st.cache_data(show_spinner=False)
def build_dataset(start_date: str, horizon_days: int, include_gdelt: bool) -> pd.DataFrame:
    df = load_wti(start=start_date)
    if include_gdelt:
        df = add_gdelt_features(df)
    else:
        # Fill GDELT columns with zeros so model frame stays consistent
        df["energy_news_count"] = 0.0
        df["energy_news_tone"] = 0.0
        df["news_spike_ratio"] = 0.0
    df = add_technical_features(df)
    df = make_targets(df, horizon_days=horizon_days)
    df = build_model_frame(df)
    return df

df = build_dataset(start_date, horizon_days, include_gdelt)

# --- Top row: current stats + forecast ---
col1, col2, col3 = st.columns([1.3, 1, 1])

latest_price = float(df["Settle"].iloc[-1])
week_change = float((df["Settle"].iloc[-1] / df["Settle"].iloc[-6] - 1) * 100) if len(df) > 6 else np.nan

with col1:
    st.metric("Latest WTI (proxy settlement)", f"${latest_price:,.2f}", f"{week_change:+.2f}% (1w)")
...
# Simple “risk gauge” based on recent vol
risk = float(df["vol_20d"].iloc[-1])
with col3:
    st.metric("Risk (20d vol, log-return std)", f"{risk:.4f}")
...
with left:
    ...
    fig = px.line(df.reset_index(), x="Date", y="Settle", title="WTI (CL=F) Settlement Proxy")
    st.plotly_chart(fig, use_container_width=True)

with right:
    st.subheader("Geopolitical / News Pressure")
    if include_gdelt:
        g = df.reset_index()
        fig2 = px.line(g, x="Date", y="energy_news_count", title="GDELT matching-article count (daily)")
        st.plotly_chart(fig2, use_container_width=True)
        fig3 = px.line(g, x="Date", y="news_spike_ratio", title="News spike ratio (count / 30d mean)")
        st.plotly_chart(fig3, use_container_width=True)
    else:
        st.info("Enable GDELT signals in the sidebar to see event pressure features.")
